...
const EventsCarouselTwo: React.FC<HomeCarouselProps> = ({ style_2, style_3 }) => {
  ...
  const title = locale === 'sv' ? 'företag' : 'corporate';

  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}events?locale=${locale}&title=${title}&limit=8locale=${locale}`, {
          headers: {
